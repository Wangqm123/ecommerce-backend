...
async function trigger({ startDate, endDate, params = {} }) {
  ...
  const [[task]] = await pool.execute('SELECT * FROM association_tasks WHERE batch_uuid = ?', [batchUuid]);
...
async function getStatus(batchUuid) {
  const [rows] = await pool.execute('SELECT * FROM association_tasks WHERE batch_uuid = ?', [batchUuid]);
